...
export default function ResellerSupportEmplyees() {
  ...
  const getSearchReseller = async (name) => {
    ...
    try {
      const response = await fetch(`https://masa-system.vercel.app/api/v1/reseller/search?name=${name}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `sysOM0${localStorage.getItem('authToken')}`
      ...
      <section className='pl-10 pr-10 mb-20'>
        ...
        <div className={`${styles.resellerSupport_table} `}>
          ...
          <table className='table-auto w-full mt-4'>
            ...
            <tbody>
              {isAllReseller.map((resellers, index) => (
                <tr key={index}>
                  <td scope="col" className="py-4 text-[#3E3D3D]">{resellers.name}</td>
                  <td scope="col" className="py-3 text-[#3E3D3D]">
                    <span className='bg-black w-20 h-7 inline-block rounded-lg text-white'>{resellers.totalAccountNum}</span>
                  </td>
                  <td scope="col" className="py-3 text-[#3E3D3D]">
                    <span className='bg-black w-20 h-7 inline-block rounded-lg text-white'>{resellers.totalDeviceNum}</span>
                  </td>
