...
export default function Customers() {
  ...
  const getSearchCustomer = async (macAddress) => {
    ...
    try {
      const response = await fetch(`https://masa-system.vercel.app/api/v1/customer/search?macAddress=${macAddress}&employee=Admin`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `sysOM0${localStorage.getItem('authToken')}`
      ...
      <div className={`${styles.customers_table}`}>
        ...
        <table className='table-auto w-full mt-4'>
          <thead className='bg-white border-b dark:bg-gray-800 dark:border-gray-700 '>
            <tr className={`${styles.customers_header}`}>
