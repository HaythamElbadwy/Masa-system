...
export default function CustomerSupportEmplyees() {
  ...
  const getCustomer = async (page, isAppFilter, isProviderFilter, isDateFilter, isStatueFilter) => {
    ...
    try {
      const response = await fetch(`https://masa-system.vercel.app/api/v1/customer/get?page=${page}&app=${isAppFilter}&statue=${isStatueFilter}&provider=${isProviderFilter}&date=${isDateFilter}&employee=Support`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `sysOM0${localStorage.getItem('authToken')}`
