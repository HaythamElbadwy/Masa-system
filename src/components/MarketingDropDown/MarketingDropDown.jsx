...
export default function MarketingDropDown() {
  ...
  const getCustomer = async (page, employeeId, isAppFilter, isProviderFilter, from, to , isStatueFilter) => {
    ...
    try {
      const response = await fetch(`https://masa-system.vercel.app/api/v1/customer/get?page=${page}&app=${isAppFilter}&statue=${isStatueFilter}&provider=${isProviderFilter}&from=${from}&to=${to}&employee=Marketing&employeeId=${employeeId}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `sysOM0${localStorage.getItem('authToken')}`
          ...
          <thead className='bg-white border-b dark:bg-gray-800 dark:border-gray-700 '>
            <tr className={`${styles.reseller_header}`}>
              <th scope="col" className="py-3">
                <select
                  className="border w-[128px] bg-[#d1d1d1] border-gray-300 rounded  py-1 text-sm focus:outline-none"
